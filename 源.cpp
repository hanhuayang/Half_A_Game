...
void DrawPiece()
{
	for (size_t i = 0; i < 3; i++)
	{
		for (size_t j = 0; j < 3; j++)
		{
			...
			{
			case 'O':
				circle(200 * j + 100, 200 * i + 100, 100);//圆心=矩形左上角坐标在水平和竖直方向都加上100像素单位
				break;
			case'X':
...
void DrawTipText()
{
	static TCHAR str[64];//与printf和sprintf很像，只不过为了在更加通用的编程环境下使用
	...
	while (running)
	{
		...
		while (peekmessage(&msg))
		{
			if (msg.message == WM_LBUTTONDOWN)
			{
				...
				if (board_data[index_y][index_x] == '-')//检查这个位置的数组元素是否为'-'字符，避免修改已有棋子的网格
				{
					board_data[index_y][index_x] = current_piece;//把当前位置元素赋值为当前棋子类型
